...
from .serializers import FreezerSensorDataSerializer
...
@extend_schema(
    ...
    responses={201: FreezerSensorDataSerializer},
)
@api_view(['POST'])
...
def create_freezer_sensor_data(request):
    ...
    return Response(FreezerSensorDataSerializer(instance).data, status=status.HTTP_201_CREATED)


@extend_schema(
    tags=['Freezer data'],
    summary='Get latest telemetry globally',
    responses={200: FreezerSensorDataSerializer},
)
@api_view(['GET'])
@permission_classes([AllowAny])
def get_last_freezer_sensor_data(request):
    ...
    return Response(FreezerSensorDataSerializer(last_record).data, status=status.HTTP_200_OK)
